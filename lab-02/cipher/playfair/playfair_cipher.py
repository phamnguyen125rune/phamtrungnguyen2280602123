class PlayfairCipher:
        ...
        while i < len(plain_text):
            if i + 1 < len(plain_text):
                if plain_text[i] == plain_text[i+1]:
                    ...
                    processed_plain_text += plain_text[i:i+2]
                    i += 2
            else:
                processed_plain_text += plain_text[i] + 'X' # Add 'X' if odd length
                i += 1
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        for i in range(0, len(decrypted_text) - 1, 2):
            if decrypted_text[i+1] == "X" and (i + 2 >= len(decrypted_text) or decrypted_text[i] == decrypted_text[i+2]):
